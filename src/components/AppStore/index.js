...
class AppStore extends Component {
  ...
  updateTabItemId = tabId => {
    this.setState({activeTabId: tabId})
  }

  render() {
    const filteredAppItems = this.getFilteredAppItems()
    const {activeTabId} = this.state
    const {searchInput} = this.state

    const searchResults = filteredAppItems.filter(eachOne =>
        ...
        <ul className="TabItemsContainer">
          {tabsList.map(eachTabItem => (
            <TabItem
              ...
              updateTabItem={this.updateTabItemId}
